...
public class GiveDamageToPlayer : MonoBehaviour {
    ...
    public void OnTriggerEnter2D(Collider2D other)
    {
        ...
        controller.SetForce(new Vector2(
            -1 * Mathf.Sign(totalVelocity.x) * Mathf.Clamp(Mathf.Abs(totalVelocity.x) * 3, 5, 15),
            -1 * Mathf.Sign(totalVelocity.y) * Mathf.Clamp(Mathf.Abs(totalVelocity.y) * 1.5f, 2.5f, 7.5f)));
    }
}
